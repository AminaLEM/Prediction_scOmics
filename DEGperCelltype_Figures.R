...
cohort <-
  LoadH5Seurat(
    "~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/output_celltype/merged_training/pseudo_bulk.h5Seurat"
  )
...
pdf(
  file = paste0(
    "~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/figures/cpm.pdf"
  ),
...
for (ct in all_celltypes) {
  ...
  pdf(file = paste0(
    paste0(
      "~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/figures/edgeR_",
      ct
    ),
    '.pdf'
  ),
  ...
  write.csv(as.data.frame(resedger$table), paste0('~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/figures/edgeR',ct))
  ...
  write.csv(res2,paste0('~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/figures/Deseq',ct))
  res2 = res2[abs(res2$log2FoldChange) >= 2 & res2$padj < 0.05, ]
  
  pdf(file = paste0(
    paste0(
      "~/new_NUM_CODEX_PLUS/Amina/CellSubmission/new_runs/figures/",
      ct
    ),
    '.pdf'
  ),
