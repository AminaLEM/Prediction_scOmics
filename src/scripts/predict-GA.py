...
def shap_loop(model_j, training_set,dim_exp, dim_cells,x_exp,x_cell):
    ...
    for i in range(model_len):
        ...
        explainer = shap.DeepExplainer(model_joint, [x_ref_exp,x_ref_cell,x_ref_gender])
        shap_values = explainer.shap_values([np.array(x_exp),np.array(x_cell),np.concatenate((gender_dummies,age_), axis=1)])
...
if __name__ == "__main__":
    ...
    with open(model_j_f+'selected_cols.pkl', 'rb') as b:
        selected_cols=pickle.load(b)
    # prepare data
    ...
    # run prediction 
    y_score1_RF=predict_loop(RF_j,np.concatenate((gender_dummies,age_,x_exp,x_cell), axis=1))
    y_score2_RF=predict_loop(RF_e,np.concatenate((gender_dummies,age_,x_exp), axis=1))
    y_score3_RF=predict_loop(RF_c,np.concatenate((gender_dummies,age_,x_cell), axis=1))
    
    
    y_score1=predict_loop(model_j,[x_exp,x_cell,np.concatenate((gender_dummies,age_), axis=1)])
    y_score2=predict_loop(model_e,np.concatenate((gender_dummies,age_,x_exp), axis=1))
    y_score3=predict_loop(model_c,np.concatenate((gender_dummies,age_,x_cell), axis=1))

    y_score1_svm=predict_loop(svm_j,np.concatenate((gender_dummies,age_,x_exp,x_cell), axis=1))
    y_score2_svm=predict_loop(svm_e,np.concatenate((gender_dummies,age_,x_exp), axis=1))
    y_score3_svm=predict_loop(svm_c,np.concatenate((gender_dummies,age_,x_cell), axis=1))

    y_score1_LReg=predict_loop(LReg_j,np.concatenate((gender_dummies,age_,x_exp,x_cell), axis=1))
    y_score2_LReg=predict_loop(LReg_e,np.concatenate((gender_dummies,age_,x_exp), axis=1))
    y_score3_LReg=predict_loop(LReg_c,np.concatenate((gender_dummies,age_,x_cell), axis=1))

    y_score1_LogReg=predict_loop(LogReg_j,np.concatenate((gender_dummies,age_,x_exp,x_cell), axis=1))
    y_score2_LogReg=predict_loop(LogReg_e,np.concatenate((gender_dummies,age_,x_exp), axis=1))
    y_score3_LogReg=predict_loop(LogReg_c,np.concatenate((gender_dummies,age_,x_cell), axis=1))
...
#         plt.show()
    ...
    shap_values_all_exp,shap_values_all_cell,shap_values_all_gender=shap_loop(list(model_j), training_set,dim_exp, dim_cells,x_exp,x_cell)
    ...
    f6 = plt.figure()
    with plt.rc_context({'figure.figsize': (4, 3), 'figure.dpi':300}):
        shap.summary_plot(shap_values_all_gender/nb, plot_type= 'violin',features=np.concatenate((gender_dummies,age_), axis=1)
                          , feature_names =['Male','Female','Age'],color_bar_label='Feature value',show=False, max_display=15)
