...
def build_2modalities_classifier(inp1, inp2):
    ...
    w = tf.keras.layers.Dense(1, activation='relu',kernel_initializer= tf.keras.initializers.GlorotNormal(seed=1234))(inputC)
...
def task(bs_index_list_stratified):
        ...
        model, res_task['history']= training(model, [train[:,3:dim_exp+3],train[:,dim_exp+3:(dim_exp+dim_cells+3)],train[:,:3]]
                                 , train[:,-1],([test[:,3:dim_exp+3],test[:,dim_exp+3:(dim_exp+dim_cells+3)], test[:,:3]],test[:,-1]),ref)
        ...
        train_set=np.concatenate((train[:,:3],train[:,dim_exp+3:(dim_exp+3+dim_cells)]), axis=1)
        test_set=np.concatenate((test[:,:3],test[:,dim_exp+3:(dim_exp+3+dim_cells)]), axis=1)
...
if __name__ == "__main__":
    ...
    # Select dependent celltypes to the target using ANOVA Fscore                    
